...
def get_combined_services():
    ...
    try:
        admin_response_layers = requests.get(f"{request.host_url}services/admin?portal={portal}").json()
        admin_response_theme = requests.get(f"{request.host_url}services/admin/theme?portal={portal}").json()
    except:
        admin_response_layers = None
        admin_response_theme = None

    print (admin_response_theme)
    for item in gs_response:
        flag = False  # Reset flag for each item in gs_response
        if admin_response_layers:
            for layer in admin_response_layers:
...
def get_layer_thema(admin_response_theme):
    pass
...
def get_theme_admin():
    ...
    # If admin is not integrated
        ...
        admin_layers_URL = f"{BASE_URL}/api/admin/layers/theme"
        admin_response = requests.get(admin_layers_URL, headers=HEADERS)
        if admin_response.status_code != 200:
        ...
        try:
            ...
            print(f"Error parsing JSON response for admin themas: {e}")
            return jsonify(result)
